            ...
            <div class="row mb-2">
            <div class="col-sm-6">
            <h1 class="m-0 text-dark">Update Article  {{this.$route.params.idAnnonce}}</h1>
            </div><!-- /.col -->
            <div class="col-sm-6">
            <ol class="breadcrumb float-sm-right">
            <li class="breadcrumb-item"><a href="#">Home</a></li>
            <li class="breadcrumb-item active">Update Annonce</li>
            </ol>
            </div><!-- /.col -->
            </div><!-- /.row -->
            </div><!-- /.container-fluid -->
            ...
            <h3 class="card-title">Update Annonce</h3>
            </div>
            <!-- /.card-header -->
            <!-- form start -->
...
export default {
 ...
 mounted() {
           this.$store.dispatch("allCategory");
            axios.get(`/editAnnonces/${this.$route.params.idAnnonce}`)//had articleid jaya mn route
            .then((response)=>{
                this.form.fill(response.data.annonces)//had article lihna jaya mn lif controller fossst json
            })
        },
    computed:{
        getallCategory(){
    ...
    methods:{
        ...
        updateArticle(){
            //this.$Progress.start();
            this.form.post(`/updateAnnonce/${this.$route.params.idAnnonce}`)
            .then((response)=>{
                //afficheCategory()
